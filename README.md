...
| 🧬 Процессы | Все процессы дерева песочницы — что именно запустил анализируемый файл. |
...
| 📄 Отчёт | `report.html`, `report.json` и `iocs.txt` — переносимый список индикаторов. |
...
| 🧬 Processes | Every process of the sandbox tree — what the analysed file actually launched. |
...
| 📄 Report | `report.html`, `report.json` and `iocs.txt` — a portable indicator list. |
